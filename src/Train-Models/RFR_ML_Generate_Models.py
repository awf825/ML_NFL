...
# datasets = ["games_1979-2024","games_1988-2024","games_1996-2024","games_2003-2024","games_2013-2024"]
...
for dataset in datasets:
    ...
    ref_regressor = RandomForestRegressor(random_state=42)
    ...
    rfr = RandomForestRegressor()# Instantiate the grid search model
    print('Begin 10 fold GridSearchCV...')
    grid_search = GridSearchCV(estimator = rfr, param_grid = param_grid, cv = 10, n_jobs = -1, verbose = 2)
    ...
    cross_val = np.mean(cross_val_score(rfr, X_train, y_train, cv=10))

    print('STATS FOR FINAL MODEL: ')
    ...
    print('Saving model...')
    with open(
        f'/Users/aidenflynn/ML_Python/python-nfl/Models/RF/Regressor/ML/{year}_{month}_{day}_{round(cross_val*100,1)}_R2_{round(r2*100, 1)}_{dataset}.obj', 
        'wb'
    ) as f:
        dill.dump(rfr, f)
